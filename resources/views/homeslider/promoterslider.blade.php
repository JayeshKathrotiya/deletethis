      ...
      <div class="col-sm-12">
        <div class="card">
          <div class="card-header">
            <input type="button" class="btn btn-primary float-right" id="btn_addcat" name="btn_addcat" value="Add Promoter Slider Image" data-toggle="modal" data-target="#addshape_model" onclick="javascript:setNull()">
            <h5 class="card-title mb-0">List All Promoter Slider Image</h5>
          </div>
          <div class="card-body">
                  ...
                  <thead>
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td>{{ $value->name }}</td>
                        <td><img src="@if(!empty($value->image)) {{ asset('promoter_slider_img/'.$value->image.'')}} @else {{ asset('class_logo/'.$value->class_logo.'')}} @endif" hight="50px" width="70px"></td>
                        <td>  
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="cat_countryel">Add Promoter Slider Image Details</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
      </button>
      </div>
      <form action="/promoter_slider/insert" method="POST" id="homeslider" name="homeslider" enctype="multipart/form-data">
         @csrf
          <div class="modal-body">
            <div class="row">
              ...
              <div class="col-md-12">
                <div class="form-group">
                  <label>Select Image</label> <span class="req-star text-danger"> *</span>(360px * 240px)
...
<div class="modal fade" id="editshape_model" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Edit Promoter Slider Image Details</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
      </button>
  </div>
  <form action="/promoter_slider/update" method="POST" id="editslider" name="editslider" enctype="multipart/form-data">
     @csrf
      <div class="modal-body">
        <div class="row">
          <div class="col-md-12">
    ...
    $('#homeslider').validate({
        rules:
        {
            ...
            name:
            {
                required:true,
                remote:{
                   url:"{{route('promoter_slider.check_class_name')}}",
                   type:"POST",
                   dataType:"json",
                   data: 
                   {
    ...
    $('#editslider').validate({
        rules:
        {
            ...
            edit_name:
            {
                required:true,
                remote:{
                   url:"{{route('promoter_slider.check_class_name_edit')}}",
                   type:"POST",
                   dataType:"json",
                   data: 
                   {
  ...
  if(id != "" ) {
    $.confirm({
      ...
      buttons:{
        Confirm:function(){
           $.ajax({
              url:'{{ route('promoter_slider.delete_promoter_slider_data')}}',
              method:'POST',
              dataType:'JSON',
              data:{id:id},
...
function editsliderimage(id,class_id,city_id,image) 
{
  if(id != "" && class_id != "" && city_id != "") {
    ...
    var profile_url = '{{ asset('promoter_slider_img')}}';
    $('#edit_profile').attr('src',profile_url+'/'+image);
    $('#old_file').val(image);
      if(city_id != "") {
      $.ajax({
        url:'{{route('promoter_slider.fetch_class')}}',
        method:'POST',
        dataType:'JSON',
