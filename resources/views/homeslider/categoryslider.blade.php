      ...
      <div class="col-sm-12">
        <div class="card">
          <div class="card-header">
              <input type="button" class="btn btn-primary float-right" id="btn_addcat" name="btn_addcat" value="Add Category Slider Image" data-toggle="modal" data-target="#addshape_model" onclick="javascript:setNull()">
            <h5 class="card-title mb-0">List All Category Slider Image</h5>
          </div>
          <div class="card-body">
              ...
              <table id="tbl_mcat" class="table table-striped table-bordered sorting-data">
                  <thead>
                    ...
                    <tbody>
                      ...
                      <tr>
                        <td>{{ ++$key }}</td>
                        <td><img src="{{ asset('category_slider_img/'.$value->image.'')}}" hight="50px" width="70px"></td>
                        <td>  
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Edit Category Slider Image Details</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
      </button>
  </div>
  <form action="/category_slider/update" method="POST" id="editslider" name="editslider" enctype="multipart/form-data">
     @csrf
        ...
        <div class="row">
          <div class="col-md-8">
              <div class="form-group">
                ...
                <label>Select Image</label> <span class="req-star text-danger"> *</span>(1100px * 400px)
...
<div class="modal fade" id="addshape_model" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="cat_countryel">Add Category Slider Image Details</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
      </button>
      </div>
      <form action="/category_slider/insert" method="POST" id="homeslider" name="homeslider" enctype="multipart/form-data">
         @csrf
          <div class="modal-body">
            <div class="row">   
              <div class="col-md-12">
  ...
  if(id != "" && image != "") {
    $.confirm({
      ...
      buttons:{
        Confirm:function(){
           $.ajax({
              url:'{{ route('category_slider.delete_category_slider_data')}}',
              method:'POST',
              dataType:'JSON',
...
function editsliderimage(id,image) 
{
  if(id != "" && image != "") {
    setNull()
    $('#id').val(id);
    var profile_url = '{{ asset('category_slider_img')}}';
